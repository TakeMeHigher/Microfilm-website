...
class Pagination(object):
    ...
    def page_html(self):
        ...
        # 首页
        self.parmas['page'] = 1
        url=urlencode(self.parmas)
        first_page = '<li><a href="%s?%s">首页</a></li>' % (self.base_url,url)
        ...
        if self.current_page == 1:
            ...
            self.base_url, urlencode(self.parmas))
        ...
        for i in range(page_start, page_end + 1):
            ...
            if i == self.current_page:
                ...
                tmp='<li class="active"><a href="%s?%s" >%s</a></li>'%(self.base_url, urlencode(self.parmas), i)
            else:
                tmp='<li><a href="%s?%s" >%s</a></li>'%(self.base_url, urlencode(self.parmas), i)
        ...
        if self.current_page == self.max_page_num:
            ...
            self.parmas['page']=self.current_page+1
            #next_page = "<a href='%s?%s'>下一页</a>" % (self.base_url,self.parmas.urlencode())
            next_page= next_page='<li><a href="%s?%s" aria-label="Next" disabled="disabled"><span aria-hidden="true">&raquo;</span></a></li>'% (self.base_url,urlencode(self.parmas
                                                                                                                                                                        ))

        html_list.append(next_page)

        self.parmas['page']=self.max_page_num
        last_url = "<li><a href='%s?%s'>尾页</a></li>" % (self.base_url, urlencode(self.parmas))
